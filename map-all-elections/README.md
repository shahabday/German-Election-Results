# Interactive Election Map — All Election Types, Municipality Resolution
...
```bash
python -m http.server 8421 --directory map-all-elections
```
...
`map-all-elections/build_berlin_2026.py` for the scraper/parser and
`map-all-elections/data/special/berlin_2026/` for the raw pages, parsed results, and
...
See `map-all-elections/build_state_elections_2026.py`.
...
- `build_municipality_data.py` does the join and writes everything under
  ...
  ```bash
  python map-all-elections/build_municipality_data.py
  ```
