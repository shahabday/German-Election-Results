...
- It's the only Landtag election in this repo sourced live from the state's own results
  ...
  see map-all-elections/README.md).
...
Run from the repo root:
    python map-all-elections/build_berlin_2026.py
"""
import json
import re
...
RAW_DIR = ROOT / "map-all-elections" / "data" / "special" / "berlin_2026" / "raw_pages"
BOUNDARY_IN = ROOT / "map-all-elections" / "data" / "special" / "berlin_2026" / "berlin_2026_wahlkreise_raw.geojson"
OUT_DIR = ROOT / "map-all-elections" / "data" / "special" / "berlin_2026"
PREPARED_LANDTAG_DIR = ROOT / "map-all-elections" / "data" / "prepared" / "landtag"
BERLIN_AGS = "11000000"
...
# Kept consistent with map-all-elections/build_municipality_data.py's PARTY_INFO keys
...
def parse_wahlkreis_page(html_text, gebiet_nr):
    ...
    return {
        ...
        # docstring / map-all-elections/README.md for why Wahlkreis resolution is the
...
def main():
    ...
    # rest of map-all-elections uses (keyed by gebiet_nr instead of ags), so the frontend
    ...
    # everything else in map-all-elections, have a genuine single-winner seat like the
